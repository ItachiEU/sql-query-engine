...
class TestRuleBreakUpSelections(unittest.TestCase):
    ...
    def test_break_selections_example_from_paper(self):
        self._check("\project_{P.age} \select_{P.name = E.name and E.pizza = 'mushroom'} (\\rename_{P:*}(Person) \cross \\rename_{E:*}(Eats));",
                    "\project_{P.age} (\select_{P.name = E.name} (\select_{E.pizza = 'mushroom'} ((\\rename_{P: *} Person) \cross (\\rename_{E: *} Eats))));")
        
    def test_break_selections_example_from_paper_3_selects(self):
        self._check("\project_{P.age} \select_{P.name = E.name and E.pizza = 'mushroom' and E.rate = 'great'} (\\rename_{P:*}(Person) \cross \\rename_{E:*}(Eats));",
                 "\project_{P.age} (\select_{P.name = E.name} (\select_{E.pizza = 'mushroom'} (\select_{E.rate = 'great'} ((\\rename_{P: *} Person) \cross (\\rename_{E: *} Eats)))));")
